...
class Collectives {
 public:
  ...
  virtual torch::Tensor broadcast(const torch::Tensor& input, int64_t root, const std::vector<int64_t>& group) = 0;
  ...
  virtual torch::Tensor all_to_all(const torch::Tensor& input, const std::vector<int64_t>& group, int64_t dim) = 0;
