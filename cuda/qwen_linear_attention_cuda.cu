...
__global__ void qwen_linear_attn_backward_recompute_tiled_kernel(const float* __restrict__ grad_out,
                                                                 const float* __restrict__ q,
                                                                 const float* __restrict__ k,
                                                                 const float* __restrict__ v,
                                                                 const float* __restrict__ beta,
...
std::vector<torch::Tensor> qwen_linear_attention_cuda_backward_recompute(
  ...
  if (shared > 192 * 1024) {
    throw std::runtime_error("Qwen linear attention CUDA tiled recompute backward shared memory limit exceeded");
  }
  check_cuda(cudaFuncSetAttribute(qwen_linear_attn_backward_recompute_tiled_kernel,
                                  cudaFuncAttributeMaxDynamicSharedMemorySize,
                                  static_cast<int>(shared)),
             "cudaFuncSetAttribute tiled recompute backward");
  qwen_linear_attn_backward_recompute_tiled_kernel<<<dim3(B * H, value_tiles), 256, shared>>>(
      grad_out.data_ptr<float>(), query.data_ptr<float>(), key.data_ptr<float>(), value.data_ptr<float>(),
      beta.data_ptr<float>(), g.data_ptr<float>(), dq.data_ptr<float>(), dk.data_ptr<float>(),
      dv.data_ptr<float>(), dbeta.data_ptr<float>(), dg.data_ptr<float>(), B, H, T, K, V, value_tile);
  check_cuda(cudaGetLastError(), "qwen_linear_attn_backward_recompute_tiled_kernel");
  return {dq, dk, dv, dbeta, dg};
}
