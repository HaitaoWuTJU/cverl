  ...
  for (int t = T - 1; t >= 0; --t) {
    ...
    if (threadIdx.x == 0) {
      ...
      for (int j = 0; j < V; ++j) {
        ...
        float ddelta = 0.0f;
        for (int kk = 0; kk < K; ++kk) {
          ddelta += dspre[kk * V + j] * k[q_base + t * K + kk];
        }
...
std::tuple<torch::Tensor, torch::Tensor> qwen_linear_attention_cuda_forward(
  ...
  auto states = torch::empty({B, H, T, K, V}, query.options());
  ...
  qwen_linear_attn_forward_kernel<<<B * H, 256, shared>>>(
      ...
      beta.data_ptr<float>(), g.data_ptr<float>(), out.data_ptr<float>(), states.data_ptr<float>(),
      B, H, T, K, V);
